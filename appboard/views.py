...
def reply_create(request, pk):
    ...
    send_mail(
        subject=f"Новый отзыв на ваше объявление '{post}'",
        message=f"Уважаемый, {post.author}!\n\nВы получили новый отзыв на ваше объявление '{post}'.\nПримите или отклоните его.",
...
def reply_set_status(request):
    ...
    next = request.GET.get('next')
    send_mail(
        subject=f"Ответ на отзыв по объявлению '{reply.post}'",
        message=f"Уважаемый, {reply.author}!\n\nВаш отзыв'{reply}' на объявление '{reply.post}' {status}.",
        from_email=settings.DEFAULT_FROM_EMAIL,
